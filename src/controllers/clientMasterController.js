...
const parseContactNumbers = (input) => {
    if (!input) return [];
    try {
...
// Parse emails array from request body (same pattern as parseContactNumbers)
const parseEmails = (input) => {
    if (!input) return [];
    try {
        const parsed = JSON.parse(input);
        if (Array.isArray(parsed)) return parsed.map(s => s.trim().toLowerCase()).filter(Boolean);
    } catch (e) { }
    if (typeof input === 'string') {
...
const storeClientMaster = async (req, res) => {
    try {
        ...
        // Build emails array: prefer new `emails` field, fall back to legacy `email`
        const parsedEmails = parseEmails(emails);
        const primaryEmail = email ? email.trim().toLowerCase() : '';
        // Merge: if emails array provided, use it; otherwise use single email as array
        const finalEmails = parsedEmails.length > 0
            ? parsedEmails
        ...
        const record = new ClientMaster({
            ...
            clientName,
            refNo,
            email: finalEmails[0] || primaryEmail || '',   // legacy field = first email
            emails: finalEmails,                            // new multi-email field
            contactPerson: {
...
const updateClientMaster = async (req, res) => {
    try {
        ...
        const {
            clientName, refNo, email, emails, contactPersonName, contactPersonPhone, panCard, clientAddress, gstNo, msmeNo, pincode, state, contactNumbers
        } = req.body;
        const files = req.files;
        ...
        // Update emails - prefer new emails array, fall back to single email field
        if (emails !== undefined) {
            const parsedEmails = parseEmails(emails);
            const primaryEmail = (email !== undefined ? email : oldRecord.email || '').trim().toLowerCase();
            const finalEmails = parsedEmails.length > 0
                ? parsedEmails
                : (primaryEmail ? [primaryEmail] : []);
            oldRecord.emails = finalEmails;
            oldRecord.email = finalEmails[0] || '';
        } else if (email !== undefined) {
            // Legacy: only single email provided
            const primaryEmail = email.trim().toLowerCase();
            oldRecord.email = primaryEmail;
            // Keep existing emails array, just replace first entry
            if (oldRecord.emails && oldRecord.emails.length > 0) {
                oldRecord.emails[0] = primaryEmail;
            } else {
                oldRecord.emails = primaryEmail ? [primaryEmail] : [];
            }
        }
        ...
        if (pincode !== undefined) oldRecord.pincode = pincode;
        if (state !== undefined) oldRecord.state = state;
        if (contactNumbers !== undefined) oldRecord.contactNumbers = parseContactNumbers(contactNumbers);
